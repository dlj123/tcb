 require(['../config'],function(config) {
	...
	require(['jquery','common','fastclick','webuploader'], function ($, common,fastclick,webuploader) {
          ...
          var productionReward = {
              ...
              this.productionRewardSubmit();

              this.productionRewardRecord();
            ...
            productionRewardSubmit:function(){
              ...
              $('.submit_wrap').on('click','.submit_btn',function(){
                  ...
                  var wxOaDoc = {};
                      ...
                      wxOaDoc.rewardCause = selectValue02($('#rewardCause').val());

                      wxOaDoc.money = selectValue02($('#money').val());

                      wxOaDoc.formType ='scjl';
                        ...
                        $.ajax({
                            ...
                            success:function(res){
                               ...
                               if (res.status=="200") {
                                 ...
                                 window.location.href="productionReward_submit_success.html?companyId="+companyId+"&oaDocId="+oaDocId+"&formType="+wxOaDoc.formType+"&deptId="+deptId
  ...
  delUppic:function(){
           ...
           //生产扣除记录初始化
           productionRewardRecord:function(){
                ...
                $.ajax({
                    ...
                    success:function(res){
                      ...
                      var productionRewardList = res.data.wxOaDocList;

                        for (var i = productionRewardList.length-1; i >= 0; i--) {
                
                                                listHtml +='<li class="item"><a href="productionReward_record.html?oaDocId='+productionRewardList[i].id+'&companyId='+companyId+'&deptId='+deptId+'">';

                                                listHtml +=   '<div class="name">'+productionRewardList[i].nCompanyName+'</div>';

                                                listHtml +=         '<div class="leave_infos">';

                                                listHtml +=           '<div class="time_reason">';

                                                listHtml +=             '<p>部门：'+productionRewardList[i].nDeptName+'</p>';

                                                listHtml +=            '<p>人员：'+productionRewardList[i].nUserName+'</p>';

                                                listHtml +=         ' </div>';

                                                if(productionRewardList[i].applyType=='60011003'){

                                                  listHtml +=          '<span class="icon check"></span>';

                                                }
                                                else if(productionRewardList[i].applyType=='60011002'){

                                                  listHtml +=          '<span class="icon failIcon"></span>';

                                                }
                                                else if(productionRewardList[i].applyType=='60011001'){
                             ...
                             $('#productionReward_record').html(listHtml);
