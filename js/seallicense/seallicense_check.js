	...
	require(['jquery', 'common', 'fastclick'], function($, common, fastclick) {
		...
		var seallicenseCheck = {
			...
			init: function() {
				...
				this.otherFlowCheck(seallicenseCheck.companyId02,seallicenseCheck.deptId02);
			...
			toIndex:function(){
				...
				var path = window.location.search.slice(1);
				    ...
				    seallicenseCheck.companyId02 = path.companyId;

				    seallicenseCheck.deptId02 = path.deptId;
				...
				$.ajax({
					...
					success: function(res) {
					  ...
					  var fileLists = res.wxOaDoc.wxOaDocPicList,picList=[],fileList=[],picHtml="",fileHtml="";
						...
						$('.company').html(res.wxOaDoc.nCompanyName); //公司             
						$('.department').html(res.wxOaDoc.nDeptName); //部门
						$('.position').html(res.wxOaDoc.nPositionName); //职位
						$('.user').html(res.wxOaDoc.nUserName); //人员姓名
						$('.rewardDate').html(res.wxOaDoc.rewardDate); //使用时间

                        if(res.wxOaDoc.stampType=='61000001'){  //印章类型

						    $('.sealType').html("公章"); 

						}
                        else if(res.wxOaDoc.stampType=='61000002'){

                        	 $('.sealType').html("合同章"); 
                        }

                        else if(res.wxOaDoc.stampType=='61000003'){

                        	 $('.sealType').html("财务章"); 
                        }

                        else if(res.wxOaDoc.stampType=='61000004'){
                        
                             $('.sealType').html("业务章");
                        }
                        else if(res.wxOaDoc.stampType=='61000005'){
                        
                             $('.sealType').html("结算章");
                        }
                        else if(res.wxOaDoc.stampType=='61000006'){
                        
                             $('.sealType').html("法人章");

                        }

                        if(res.wxOaDoc.licenceType=='71000001'){//证照类型

						    $('.licenseType').html("三证合一正副复印件"); 

                         }
                         else if(res.wxOaDoc.licenceType=='71000002'){

                            $('.licenseType').html("道路运输许可证正副原件"); 

                         }
                         else if(res.wxOaDoc.licenceType=='71000003'){
                              
                              $('.licenseType').html("三证合一正副印件"); 

                         }
                         else if(res.wxOaDoc.licenceType=='71000004'){
                              
                              $('.licenseType').html("法人身份证复印件"); 

                         }
                         else if(res.wxOaDoc.licenceType=='71000005'){
                              
                               $('.licenseType').html("道路运输许可证正副复印件"); 
                               
                         }
						$('.uses').html(res.wxOaDoc.uses); // 用途
						$('.send_unit').html(res.wxOaDoc.sendUnit); // 发往单位
						$('.filename').html(res.wxOaDoc.fileName); //文件名称
						$('.reason').html(res.wxOaDoc.reason); // 使用事由
						$('.sealuse').html(res.wxOaDoc.sealUse); // 印章用途
						$('.returnDate').html(res.wxOaDoc.returnDate); // 归还时间
