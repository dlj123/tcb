 ...
 */
  ...
  var ipAdress = 'http://172.17.125.118:8080';
...
function AttendanceCalendar(_obj, _fn, _lable) {
   ...
   function setOnClick(span) {
    ...
    function showCurrentState() {
          ...
          $('#recordlist_wrap').hide();

           //if($('.div_calendar_body span').hasClass('today')){
              
              var shiftsId = $('.clock_tab').attr('data-shiftsid');

              var tokenId = sessionStorage.getItem("tokenId");

              var yearMonth = $('#currentMonth').html();

              var yearMonthDay = yearMonth+'.'+formatDate($('.div_calendar_body span.select').html());

                  yearMonthDay = yearMonthDay.replace(/\./g,'-');

               if(shiftsId){
  
                           console.log(tokenId,yearMonthDay,shiftsId);

                           if(shiftsId=="40011002"){

                                $('#recordlist_wrap .record_list li').css({"display":"block","border-left":"0"});

                                $('#recordlist_wrap .record_list li:not(:first)').css('display','none');
                           }
                           else{

                               $('#recordlist_wrap .record_list li').css({"display":"block","border-left":"1px dashed #f0f0f0"});

                               $('#recordlist_wrap .record_list li:not(:first)').css('border-left','0');

                           }

                          $.ajax({

                             url:ipAdress+'/weixin/shifts/selectbyday',

                             data:{

                               tokenId:tokenId,

                               date:yearMonthDay.toString(),
                             ...
                             success:function(res){
                                ...
                                console.log(res);
                                 ...
                                 if(JSON.stringify(res) == "{}" ){
                                        ...
                                        $('#recordlist_wrap').css('display','none');

                                 }
                                else if(res.Taste=="请重新登陆！"){
                                    ...
                                    alert('请重新登陆！');
                                        ...
                                        var recordValue = res.record;

                                        $('#recordlist_wrap').css('display','block');
                                        ...
                                        for(var i in recordValue){
                                            ...
                                            if(i =='beginTime'){
                                                ...
                                                $('#arriveTime').html(recordValue[i]+"(签到时间)");
                                                ...
                                                $('#backTime').html(recordValue[i]+"(签退时间)");
...
// console.log(DayNumOfMonth(2016, 2));
                   ...
                   $('.div_changeMonth').on('tap',function(){
                           ...
                           var shiftsId = $('.clock_tab').attr('data-shiftsid');
                            ...
                            var currentPicker = new mui.DtPicker({

                              "type": "month",

                              "labels":["年", "月"],

                              "beginDate":new Date(2009,1),

                              "endDate":new Date(2018,12)
                            });
                          
                           currentPicker.show(function(rs) {

                              console.log(rs.text);

                              $('#currentMonth').html(rs.text);

                               var year02 = (rs.text).slice(0,4);

                                var month02 = (rs.text).slice(5);

                                console.log(year02,month02);
                                ...
                                ac.setDate(rs.text);
                                 ...
                                 $.ajax({
                                     ...
                                     url:ipAdress+'/weixin/shifts/select',
                                     ...
                                     success:function(res){
                                          ...
                                          var recordList = res.record;

                                           var getData04 = function(ym) {

                                              var date = new Date(ym);
                                              ...
                                              var days = mGetDate(year02,month02);
                                             ...
                                             for(var i = 0;i<days;i++){
                                              ...
                                              console.log(record01);

                                             for(var j in recordList){

                                                 console.log(recordList[j].beginTime);

                                                if(recordList[j].beginTime){
                                                  
                                                  record02.push(parseInt(j));

                                                 }
                                             }
                                             console.log(record02);

                                             for(var i = 0;i <record01.length;i++){

                                                     attendances02.push({day: i, status: 0});

                                                      for(var j =  0;j < record02.length;j++){
                                                       
                                                            if((record01[i]) == record02[j]){

                                                              attendances02[i-1].status=1;
                                              ...
                                              return attendances02;

                                              console.log(attendances02);
                                          }
                                           ac.setAttendance(getData04(year02,month02));
                                     }
                                })//ajax结束                                 
                   ...
                   $('.clock_tab').on('click','a',function(){

                      var index = $(this).index();

                      if(index==0){
                          
                          $('.clock_tab a').removeClass('active').eq(0).addClass('active');

                          $('.content_panel').css('display','none').eq(0).css('display','block');
                      }
                      else if(index==1){

                          var currentDate = $('#currentMonth').html();

                          var currentDate02 = currentDate.replace(/\./,'-');

                          ac.setDate(currentDate02);

                          ac.init();

                          var year05 = currentDate.slice(0,4).toString();

                          var month05 = currentDate.slice(5).toString();

                          var shiftsId = $('.clock_tab').attr('data-shiftsid');

                          $('.clock_tab a').removeClass('active').eq(1).addClass('active');

                          $('.content_panel').css('display','none').eq(1).css('display','block');

                          console.log(year05,month05,shiftsId);
                             ...
                             success:function(res){
                                ...
                                var recordList = res.record;
                                  ...
                                  var getData04 = function(ym) {
                                              ...
                                              var days = mGetDate(year05,month05);
                                        ...
                                        console.log(currentDate02);
 
                                 ac.setAttendance(getData04(year05,month05));
