  ...
  require(['jquery','common','fastclick'], function ($, common,fastclick) {
          ...
          var basicLoan = {
            ...
            init:function(){
              ...
              this.basicLoanRecord();

              this.basicLoanSubmit();
        ...
        delUppic:function(){
            ...
            basicLoanSubmit:function(){
              ...
              $('.submit_wrap').on('click','.submit_btn',function(){
                      ...
                      wxOaDoc.turnToNumber = selectValue02($('#turnToNumber').val());

                      wxOaDoc.turnToName = selectValue02($('#turnToName').val());
                        ...
                        $.ajax({
                            ...
                            success:function(res){
                               ...
                               if (res.status=="200") {
                                 ...
                                 window.location.href="basicLoan_submit_success.html?companyId="+companyId+"&oaDocId="+oaDocId+"&formType="+wxOaDoc.formType+"&deptId="+deptId
            ...
            basicLoanRecord:function(){
                ...
                $.ajax({
                    ...
                    success:function(res){
                      ...
                      var basicLoanList = res.data.wxOaDocList;

                        for (var i = basicLoanList.length-1; i >= 0; i--) {
                
                                                listHtml +='<li class="item"><a href="basicLoan_record.html?oaDocId='+basicLoanList[i].id+'&companyId='+companyId+'&deptId='+deptId+'">';

                                                listHtml +=   '<div class="name">'+basicLoanList[i].companyName+'</div>';

                                                listHtml +=         '<div class="leave_infos">';

                                                listHtml +=           '<div class="time_reason">';

                                                listHtml +=             '<p>部门：'+basicLoanList[i].deptName+'</p>';

                                                listHtml +=            '<p>职位：'+basicLoanList[i].position+'</p>';

                                                listHtml +=            '<p>借款人：'+basicLoanList[i].userName+'</p>';

                                                listHtml +=         ' </div>';

                                                if(basicLoanList[i].applyType=='60011003'){

                                                  listHtml +=          '<span class="icon check"></span>';

                                                }
                                                else if(basicLoanList[i].applyType=='60011002'){

                                                  listHtml +=          '<span class="icon failIcon"></span>';

                                                }
                                                else if(basicLoanList[i].applyType=='60011001'){
                             ...
                             $('#basicLoan_record').html(listHtml);
