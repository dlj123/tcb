 require(['../config'],function(config) {
	...
	require(['jquery','common','fastclick','webuploader'], function ($, common,fastclick,webuploader) {
          ...
          var accidentSurvey = {
              ...
              this.accidentSurveySubmit();

              this.accidentSurveyRecord();
            ...
            dataInit:function(){
                 ...
                 $.ajax({
                    ...
                    success:function(res){
                      ...
                      $('#seePerson').html(res.tmUser.realName);
            ...
            //初始化勘察类型
            accidentTypeInit:function(){

              
               $('.accidentType_wrap').on('click',function(){

                 $('#accidentType').html("");
                
             

               var accidentTypeListNew=[],accidentTypePicker;

                $.ajax({

                    url:ipAdress+'/weixin/oaDoc/accidentType',
                    ...
                    success:function(res){
                         ...
                         if(res.status==200){
                                 ...
                                 var accidentTypeList = res.data.accidentTypeList;

                                 for(var i = 0;i<accidentTypeList.length;i++){

                                  accidentTypeListNew[i]= {};

                                  accidentTypeListNew[i].value = accidentTypeList[i].id;

                                  accidentTypeListNew[i].text = accidentTypeList[i].codeDesc;

                                 }
                                 console.log(accidentTypeListNew);

                                 if(!accidentTypePicker){

                                  accidentTypePicker = new mui.PopPicker();

                                  accidentTypePicker.setData(accidentTypeListNew);

                                  accidentTypePicker.pickers[0].setSelectedIndex(0, 1000);

                                  accidentTypePicker.show(function(getSelectedItems) {

                                      console.log(getSelectedItems[0]);

                                      $('#accidentType').html(getSelectedItems[0].text);

                                      $('.accidentType_wrap').attr('data-accidentTypeid',getSelectedItems[0].value);

                                      accidentTypePicker.dispose();
            ...
            rewardDate:function(){
              ...
              $('.rewardDate_wrap').on('click',function(){

                if(!rewardDatepicker){

                     var rewardDatepicker = new mui.DtPicker({
                    ...
                    rewardDatepicker.show(function(items) {
            ...
            accidentSurveySubmit:function(){
              ...
              $('.submit_wrap').on('click','.submit_btn',function(){
                  ...
                  var wxOaDoc = {};
                      ...
                      wxOaDoc.accidentName = $('#seePerson').html();
                     
                      wxOaDoc.place = selectValue02($('#place').val());
                      
                      wxOaDoc.accidentType = selectValue($('#accidentType').html().trim());
                      
                      wxOaDoc.money = selectValue02($('#money').val());
                      
                      wxOaDoc.witness = selectValue02($('#witness').val());
                      ...
                      if(wxOaDoc.nCompanyName.trim() =='请选择' || rewardDate.trim() =='请选择'){
                        ...
                        $.ajax({
                            ...
                            data:{
                              ...
                              rewardDate:selectValue($('#rewardDate').html())
                            ...
                            success:function(res){
                               ...
                               if (res.status=="200") {
                                 ...
                                 window.location.href="accidentSurvey_submit_success.html?companyId="+companyId+"&oaDocId="+oaDocId+"&formType="+wxOaDoc.formType+"&deptId="+deptId
            ...
            accidentSurveyRecord:function(){
                ...
                $.ajax({
                    ...
                    success:function(res){
                      ...
                      var accidentSurveyList = res.data.wxOaDocList;

                        for (var i = accidentSurveyList.length-1; i >= 0; i--) {

                                             if(accidentSurveyList[i].id){
                
                                                listHtml +='<li class="item"><a href="accidentSurvey_record.html?oaDocId='+accidentSurveyList[i].id+'&companyId='+companyId+'&deptId='+deptId+'">';
                                              }
                                              /*else if(accidentSurveyList[i].){

                                              }*/
                                                listHtml +=   '<div class="name">'+accidentSurveyList[i].nCompanyName+'</div>';

                                                listHtml +=         '<div class="leave_infos">';

                                                listHtml +=           '<div class="time_reason">';

                                                listHtml +=             '<p>部门：'+accidentSurveyList[i].nDeptName+'</p>';

                                                 listHtml +=            '<p>人员：'+accidentSurveyList[i].nUserName+'</p>';

                                                listHtml +=             '<p>日期：'+accidentSurveyList[i].rewardDate+'</p>';

                                                listHtml +=         ' </div>';

                                                if(accidentSurveyList[i].applyType=='60011003'){

                                                  listHtml +=          '<span class="icon check"></span>';

                                                }
                                                else if(accidentSurveyList[i].applyType=='60011002'){

                                                  listHtml +=          '<span class="icon failIcon"></span>';

                                                }
                                                else if(accidentSurveyList[i].applyType=='60011001'){
                             ...
                             $('#accidentSurvey_record').html(listHtml);
