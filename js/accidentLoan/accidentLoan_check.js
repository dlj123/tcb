	...
	require(['jquery','common','fastclick'], function ($, common,fastclick) {
      ...
      oaDocId:'',
			...
			init:function(){
                 ...
                 this.otherFlowCheck(accidentLoanCheck.companyId02,accidentLoanCheck.deptId02);

              },
              toIndex:function(){

                  var path = window.location.search.slice();

                      path = common.parseUrl(path);

                      accidentLoanCheck.companyId02 = path.companyId;

                      accidentLoanCheck.deptId02 = path.deptId;
        ...
        searchInit:function(){
                 ...
                 $('#search').on('click',function(){
                     ...
                     if(!accidentLoanCheck.oaDocId){
                      ...
                      window.location.href='../accidentSurvey/accidentSurvey_record.html?oaDocId='+accidentLoanCheck.oaDocId+'&companyId='+accidentLoanCheck.companyId02+'&deptId='+accidentLoanCheck.deptId02;
			...
			checkInit:function(){
                    ...
                    $.ajax({
                        ...
                        for(var i = 0;i<fileLists.length;i++){
                            ...
                            accidentLoanCheck.oaDocId = res.wxOaDoc.pAccidentId;
                            ...
                            if(!res.activitiAccredit && empower=='100011002'){
                             ...
                             $('.rewardDate').html(res.wxOaDoc.rewardDate);

                             $('.company').html(res.wxOaDoc.companyName);

                             $('.department').html(res.wxOaDoc.deptName);

                             $('.position').html(res.wxOaDoc.position);

                             $('.people').html(res.wxOaDoc.userName);

                             $('.money').html(res.wxOaDoc.money);

                             $('.amount').html(res.wxOaDoc.amount);

                             $('.reason').html(res.wxOaDoc.reason);

                             $('.memo').html(res.wxOaDoc.memo);

                            $('.enterNumber').html(res.wxOaDoc.turnToNumber);

                            $('.enterName').html(res.wxOaDoc.turnToName);
