	...
	require(['jquery', 'common', 'fastclick'], function($, common, fastclick) {
		...
		var adminMaterialsCheck = {
			...
			init: function() {
				...
				this.otherFlowCheck(adminMaterialsCheck.companyId02,adminMaterialsCheck.deptId02);
			...
			toIndex:function(){
				...
				var path = window.location.search.slice(1);
				    ...
				    adminMaterialsCheck.companyId02 = path.companyId;

				    adminMaterialsCheck.deptId02 = path.deptId;
			...
			checkInit: function() {
				...
				$.ajax({
					...
					success: function(res) {
						...
						//$('.fileName').html(res.data.wxOaDoc.fileName);// 使用类别
						$('.status').html(res.wxOaDoc.status); // 使用类别
						$('.productName').html(res.wxOaDoc.productName); //物品名称
						$('.modelType').html(res.wxOaDoc.modelType); // 规格型号
						$('.quantity').html(res.wxOaDoc.quantity); // 数量
						$('.payee').html(res.wxOaDoc.payee); //单位
						$('.price').html(res.wxOaDoc.price); // 单价
						$('.money').html(res.wxOaDoc.money); // 金额
						$('.memo').html(res.wxOaDoc.memo); //备注
