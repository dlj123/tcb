 require(['../config'], function(config) {
 	...
 	require(['jquery', 'common', 'fastclick', 'mui', 'picker'], function($, common, fastclick, mui, picker) {
      ...
      length,attachments=[];
        ...
        common.focusBug();
 				common.buttontap('#adminMaterialsSubmit');
 				common.tab('.top_tab ul li', '.content_panel');
 				this.adminMaterialsInit();
 				this.adminMaterialsRecord();
 				this.adminMaterialsSubmit();
 				this.typeSelect();
        this.delUppic();
        this.uploadPic();
 				this.toIndex();
       ...
       delUppic:function(){
 			...
 			adminMaterialsSubmit: function() {
 				...
 				$('#adminMaterialsSubmit').on('click', function() {
 					var wxOaDoc = {};
 					wxOaDoc.formType = 'xzwzcg';
 					...
 					wxOaDoc.productName = selectValue02($('.productName').val().trim()); //物品名称
 					wxOaDoc.modelType = selectValue02($('.modelType').val().trim()); //规格型号
 					wxOaDoc.quantity = selectValue02($('.quantity').val().trim()); //数量
 					wxOaDoc.payee = selectValue02($('.payee').val().trim()); //单位
 					wxOaDoc.price = selectValue02($('.price').val().trim()); //单价
 					wxOaDoc.money = selectValue02($('.money').val().trim()); //金额
 					wxOaDoc.memo = selectValue02($('.memo').val().trim()); //备注
 					wxOaDoc.turnToNumber = selectValue02($('.enterNumber').val().trim());
 					wxOaDoc.turnToName = selectValue02($('.enterName').val().trim());
          ...
          wxOaDoc.wxOaDocPicList = files;
 					console.log(tokenId, wxOaDoc.title, wxOaDoc.status, wxOaDoc.productName, wxOaDoc.modelType, wxOaDoc.quantity, wxOaDoc.payee, wxOaDoc.price, wxOaDoc.money, wxOaDoc.memo);
 					...
 					if(wxOaDoc.title.trim() == '') {
 						...
 						$.ajax({
 							...
 							success: function(res) {
 								...
 								if(res.status == 200) {
 									window.location.href = "../adminMaterials/adminMaterials_submit_success.html?companyId=" + companyId + "&oaDocId=" + oaDocId + "&formType=" + wxOaDoc.formType+'&deptId='+deptId
 								}
 							},
 							complete:function(){
 			...
 			adminMaterialsRecord: function() {
 				...
 				$.ajax({
 					...
 					success: function(res) {
 						console.log(res);
 						var adminMaterialsList = res.data.wxOaDocList;
 						for(var i = adminMaterialsList.length - 1; i >= 0; i--) {
 							listHtml += '<li class="item"><a href="adminMaterials_record.html?oaDocId=' + adminMaterialsList[i].id + '&companyId='+companyId+'&deptId='+deptId+'">';
 							//listHtml +=   '<div class="name">' +adminMaterialsList[i].companyName + '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+adminMaterialsList[i].deptName+'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+adminMaterialsList[i].userName+'</div>';
 							listHtml += '<div class="name">' + adminMaterialsList[i].companyName + '</div>';
 							listHtml += '<div class="leave_infos">';
 							listHtml += '<div class="time_reason">';
 							listHtml += '<p>部门：' + adminMaterialsList[i].deptName + '</p>';
 							listHtml += '<p>人员：' + adminMaterialsList[i].userName + '</p>';
 							listHtml += '<p>物品名称：' + adminMaterialsList[i].productName + '</p>';
 							listHtml += '<p>申请金额：' + adminMaterialsList[i].money + '元</p>';
 							if(adminMaterialsList[i].status == '81000001') {
 								listHtml += '<p>使用类别：自损配件</p>';
 							} else if(adminMaterialsList[i].status == '81000002') {
 								listHtml += '<p>使用类别：保养材料</p>';
 							} else if(adminMaterialsList[i].status == '81000003') {
 								listHtml += '<p>使用类别：低耗配件</p>';
 							}

 							listHtml += ' </div>';
 							if(adminMaterialsList[i].applyType == '60011003') {
 								listHtml += '<span class="icon check"></span>';
 							} else if(adminMaterialsList[i].applyType == '60011002') {
 								listHtml += '<span class="icon failIcon"></span>';
 							} else if(adminMaterialsList[i].applyType == '60011001') {
 								listHtml += '<span class="icon successIcon"></span>';
 							}
 							listHtml += '</div>';
 							listHtml += '</a></li>';
 						}
 						$('#adminMaterials_record').addClass('license_record').html(listHtml);
 					}
 				})
 			}
 		}
