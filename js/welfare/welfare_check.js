	...
	require(['jquery', 'common', 'fastclick'], function($, common, fastclick) {
		...
		var welfareCheck = {
			...
			init: function() {
				...
				this.otherFlowCheck(welfareCheck.companyId02,welfareCheck.deptId02);
			...
			toIndex:function(){
				...
				var path = window.location.search.slice(1);
				    ...
				    welfareCheck.companyId02 = path.companyId;

				    welfareCheck.deptId02 = path.deptId;
			...
			checkInit: function() {
				...
				$.ajax({
					...
					success: function(res) {
						...
						$('.driverName').html(res.wxOaDoc.userName);
						$('.rewardDate').html(res.wxOaDoc.rewardDate); // 福利日期
						$('.applyAmount').html(res.wxOaDoc.money); // 申请金额
						$('.enterNumber').html(res.wxOaDoc.turnToNumber);
						$('.enterName').html(res.wxOaDoc.turnToName);
						if(res.wxOaDoc.status=="51000001"){// 福利类型

							$('.welfareType').html("困难求助"); 
						}
						else if(res.wxOaDoc.status=="51000002"){

                            $('.welfareType').html("婚嫁礼金"); 
						}
						else if(res.wxOaDoc.status=="51000003"){

                            $('.welfareType').html("结婚纪念金"); 
						}
						else if(res.wxOaDoc.status=="51000004"){

                            $('.welfareType').html("慰问金"); 
						}
						else if(res.wxOaDoc.status=="51000005"){

                            $('.welfareType').html("生日礼金"); 
						}
						$('.applyReason').html(res.wxOaDoc.rewardCause); // 申请原因
						//$('.carNum').html(res.wxOaDoc.carNo);
						//$('.overSpeed_date').html(res.wxOaDoc.speedDate);
