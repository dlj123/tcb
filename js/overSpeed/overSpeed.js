 require(['../config'],function(config) {
	...
	require(['jquery','common','fastclick','webuploader'], function ($, common,fastclick,webuploader) {
          ...
          var overSpeed = {
              ...
              this.overSpeedSubmit();

              this.overSpeedRecord();
            ...
            overSpeedSubmit:function(){
              ...
              $('.submit_wrap').on('click','.submit_btn',function(){
                  ...
                  var wxOaDoc = {};
                      ...
                      wxOaDoc.money = selectValue02($('.overSpeedMark input').val());
                      ...
                      var speedDate = selectValue($('#overTime').html());

                      console.log(wxOaDoc);

                      if(wxOaDoc.nCompanyName.trim() =='请选择' || speedDate.trim() =='请选择'){
                        ...
                        $.ajax({
                            ...
                            success:function(res){
                               ...
                               if (res.status=="200") {
                                 ...
                                 window.location.href="overSpeed_submit_success.html?companyId="+companyId+"&oaDocId="+oaDocId+"&formType="+wxOaDoc.formType+"&deptId="+deptId
            ...
            overSpeedRecord:function(){
                ...
                $.ajax({
                    ...
                    success:function(res){
                      ...
                      var overSpeedList = res.data.wxOaDocList;

                        for (var i = overSpeedList.length-1; i >= 0; i--) {
                
                                                listHtml +='<li class="item"><a href="overSpeed_record.html?oaDocId='+overSpeedList[i].id+'&companyId='+companyId+'&deptId='+deptId+'">';

                                                listHtml +=   '<div class="name">'+overSpeedList[i].nCompanyName+'</div>';

                                                listHtml +=         '<div class="leave_infos">';

                                                listHtml +=           '<div class="time_reason">';

                                                listHtml +=             '<p>部门：'+overSpeedList[i].nDeptName+'</p>';

                                                 listHtml +=            '<p>人员：'+overSpeedList[i].nUserName+'</p>';

                                                listHtml +=             '<p>超速日期：'+overSpeedList[i].speedDate+'</p>';

                                                listHtml +=         ' </div>';

                                                if(overSpeedList[i].applyType=='60011003'){

                                                  listHtml +=          '<span class="icon check"></span>';

                                                }
                                                else if(overSpeedList[i].applyType=='60011002'){

                                                  listHtml +=          '<span class="icon failIcon"></span>';

                                                }
                                                else if(overSpeedList[i].applyType=='60011001'){
                             ...
                             $('#overSpeed_record').html(listHtml);
