          ...
          var reimbursement = {
            ...
            dataInit:function(){
                 ...
                 $.ajax({
                  ...
                  $('#addDetails').on('click',function(){
                    var s = "<div class='item'><div class='label'>预算内费用借款</div><div class='info_item overSpeedMark'><input name='inLoan' type='text' placeholder='请输入'' class='leaveMark'></input></div></div>";
                    s += "<div class='item'><div class='label'>预算外费用借款</div><div class='info_item overSpeedMark'><input name='outLoan' type='text' placeholder='请输入'' class='leaveMark'></input></div></div>";
                    s += "<div class='item'><div class='label'>金额</div><div class='info_item overSpeedMark'><input name='money' value='0' type='text' placeholder='请输入'' class='leaveMark'></input></div></div>";
                    s += "<div class='item'><div class='label'>打入姓名</div><div class='info_item overSpeedMark'><input name='turnToName' type='text' placeholder='请输入'' class='leaveMark'></input></div></div>";
                    s += "<div class='item'><div class='label'>打入卡号</div><div class='info_item overSpeedMark'><input name='turnToNumber' type='text' placeholder='请输入'' class='leaveMark'></input></div></div>";
                    s += "<div class='item beizhu'><div class='label'>摘要</div><div class='info_item overSpeedMark leaveMark_wrap'><textarea name='roundup' placeholder='请输入'' class='leaveMark'></textarea></div></div>";
                    $(".infos_list").append(s);
                  });

                  $(".infos_list.ltMoney").on('change',"input[name='money']",function(){

                    var amount = 0;

                    var els =document.getElementsByName("money");

                    for (var i = 0, j = els.length; i < j; i++){

                      amount += parseInt(els[i].value);
                    }
                    var s = "&nbsp;&nbsp;&nbsp;总金额:"+amount+"<br/>";
            ...
            reimbursementSubmit:function(){
              ...
              $('.submit_wrap').on('click','.submit_btn',function(){
                      ...
                      wxOaDoc.nCompanyName = companyName;

                      wxOaDoc.nDeptName = deptName;

                      wxOaDoc.nPositionName = positionName;

                      wxOaDoc.nUserName = staffName;

                      wxOaDoc.money = selectValue02($('#hiddenAmount').val());

                      var oaDocBxList = new Array();
                      

                      var inLoanArr =document.getElementsByName("inLoan");
                      var outLoanArr =document.getElementsByName("outLoan");
                      var moneyArr =document.getElementsByName("money");
                      var turnToNameArr =document.getElementsByName("turnToName");
                      var turnToNumberArr =document.getElementsByName("turnToNumber");
                      var roundupArr =document.getElementsByName("roundup");

                      for (var i = 0, j = inLoanArr.length; i < j; i++){
                        var oaDocBx = {};
                        oaDocBx.inLoan = inLoanArr[i].value;
                        oaDocBx.outLoan = outLoanArr[i].value;
                        oaDocBx.money = moneyArr[i].value;
                        oaDocBx.turnToName = turnToNameArr[i].value;
                        oaDocBx.turnToNumber = turnToNumberArr[i].value;
                        oaDocBx.roundup = roundupArr[i].value;
                        
                        oaDocBxList.push(oaDocBx);
                      }

                      wxOaDoc.oaDocBxList = oaDocBxList;
                      
                      //alert(JSON.stringify(wxOaDoc));
                      ...
                      var j = inLoanArr.length;
                      for (var i = 0; i < inLoanArr.length; i++) {
                        
                        if(inLoanArr[i].value == ''){
                          mui.alert('预算内费用借款不能为空');
                          j--;
                        }else if(outLoanArr[i].value == ''){
                          mui.alert('预算外费用借款不能为空');
                          j--;
                        }else if(moneyArr[i].value == ''){
                          mui.alert('金额不能为空');
                          j--;
                        }else if(turnToNameArr[i].value == ''){
                          mui.alert('打入姓名不能为空');
                          j--;
                        }else if(turnToNumberArr[i].value == ''){
                          mui.alert('打入卡号不能为空');
                          j--;
                        }
                      }
                      if(j==inLoanArr.length){
                         ...
                         $.ajax({
                            ...
                            success:function(res){
                               ...
                               if (res.status=="200") {
                                 ...
                                 window.location.href="reimbursement_submit_success.html?companyId="+companyId+"&oaDocId="+oaDocId+"&formType="+wxOaDoc.formType+'&deptId='+deptId
            ...
            reimbursementRecord:function(){
                ...
                $.ajax({
                    ...
                    success:function(res){
                      ...
                      var reimbursementList = res.data.wxOaDocList;

                        for (var i = reimbursementList.length-1; i >= 0; i--) {
                
                                                listHtml +='<li class="item"><a href="reimbursement_record.html?oaDocId='+reimbursementList[i].id+'&companyId='+companyId+'&deptId='+deptId+'">';

                                                listHtml +=   '<div class="name">'+reimbursementList[i].nCompanyName+'</div>';

                                                listHtml +=         '<div class="leave_infos">';

                                                listHtml +=           '<div class="time_reason">';

                                                listHtml +=             '<p>部门：'+reimbursementList[i].nDeptName+'</p>';

                                                listHtml +=            '<p>职位：'+reimbursementList[i].nPositionName+'</p>';

                                                listHtml +=            '<p>借款人：'+reimbursementList[i].nUserName+'</p>';

                                                listHtml +=         ' </div>';

                                                if(reimbursementList[i].applyType=='60011003'){

                                                  listHtml +=          '<span class="icon check"></span>';

                                                }
                                                else if(reimbursementList[i].applyType=='60011002'){

                                                  listHtml +=          '<span class="icon failIcon"></span>';

                                                }
                                                else if(reimbursementList[i].applyType=='60011001'){
                             ...
                             $('#reimbursement_record').html(listHtml);
